...
const visamaster = require('../../assets/images/visa-master-2.png');
...
export default class AccountScreen extends Component {
    ...
    place_monetbil = () => {
        const url = 'https://api.monetbil.com/payment/v1/placePayment';
        fetch(url, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
            ...
            body: JSON.stringify({
                service: '0XwOtA0ltYWVMY9IO29tSNTwJrveDWRn',
                phonenumber: '237654088375',
                // phonenumber: '15204063923',
                amount: this.state.amount_monetbil
            })
            ,
        }).then((response) => response.json())
            .then((res => {
                this.check_monetbil(res.paymentId);
            })
            )

    }
    check_monetbil = (val) => {
        const url = 'https://api.monetbil.com/payment/v1/checkPayment';
        fetch(url, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
        ...
        return (
            <Container style={styles.container}>
                <Content style={{ width: '100%' }}>
                        ...
                        <DialogContent>
                            {
                                <View style={{ width: '100%' }}>
                                    {/* <LinearGradient colors={['#00ff00', '#00ff00']} style={styles.linearGradient1}>
                                    ...
                                    <LinearGradient colors={['#800080', '#800080']} style={styles.linearGradient1}>
                                    ...
                                    <LinearGradient colors={['#00ff00', '#00ff00']} style={styles.linearGradient1}>
                                        <TouchableOpacity onPress={() => { this.setState({ choose_payment_show: false, monetbil_payment_dialog: true }) }} style={styles.buttonStyle}>
                                            <ActivityIndicator style={[styles.buttonText, { display: this.state.showing_activity !== 'flex' ? 'none' : 'flex' }]} size="small" color="#ffffff">
                                            </ActivityIndicator>
                                            <Text style={[styles.buttonText, { display: this.state.showing_activity == 'flex' ? 'none' : 'flex' }]}>
                                                Monetbil
                    ...
                    <Dialog
                        onTouchOutside={() => {
                            this.setState({ monetbil_payment_dialog: false, showwaiting_cardbtn: 'none' });
                        }}
                        width={0.9}
                        visible={this.state.monetbil_payment_dialog}
                        dialogAnimation={new ScaleAnimation()}
                        onHardwareBackPress={() => {
                            console.log('onHardwareBackPress');
                            this.setState({ monetbil_payment_dialog: false });
                            return true;
                        }}
                        actions={[
                            <DialogButton
                        ...
                        <DialogContent>
                            {
                                <View style={{ width: '100%' }}>
                                    <View style={styles.cardpart}>
                                        <View style={{ flexDirection: 'row', justifyContent: 'center', borderColor: 'gray', height: 50, borderWidth: 0.5, width: '100%', padding: 0 }}>
                                            <View style={{ width: '50%', }}>
                                                <TextInput
                                                    style={[styles.inputvalue, { textAlign: 'center' }]}
                                                    placeholder="Please Input Amount"
                                                    maxLength={20}
                                                    onChangeText={(amount_monetbil) => this.setState({ amount_monetbil })}
                                        ...
                                        <LinearGradient colors={['#00ff00', '#00ff00']} style={styles.linearGradient1}>
                                            <TouchableOpacity onPress={() => { this.setState({ monetbil_payment_dialog: false }); this.place_monetbil() }} style={styles.buttonStyle}>
                    ...
                    <View style={{ height: 30 }}></View>
                    <Text style={[styles.buttonText, { color: 'black', width: '100%', textAlign: 'center' }]}>
                        Add your payment method
                    </Text>
                    <LinearGradient colors={['#00ff00', '#00ff00']} style={styles.linearGradient1}>
                        <TouchableOpacity onPress={() => { this.setState({ ach_payment_dialog: true }) }} style={styles.buttonStyle}>
                            <ActivityIndicator style={[styles.buttonText, { display: this.state.showing_activity !== 'flex' ? 'none' : 'flex' }]} size="small" color="#ffffff">
                            </ActivityIndicator>
                            <Text style={[styles.buttonText, { display: this.state.showing_activity == 'flex' ? 'none' : 'flex' }]}>
                                US ACH
                    ...
                    <View style={{ height: 30 }}></View>
                    <LinearGradient colors={['#00ff00', '#00ff00']} style={styles.linearGradient1}>
                        <TouchableOpacity onPress={() => { this.setState({ card_payment_dialog: true }) }} style={styles.buttonStyle}>
                            <ActivityIndicator style={[styles.buttonText, { display: this.state.showing_activity !== 'flex' ? 'none' : 'flex' }]} size="small" color="#ffffff">
                            </ActivityIndicator>
                            <Image source={visamaster} style={{ width: 60, height: 20, resizeMode: 'contain', marginTop: 0, display: this.state.showing_activity == 'flex' ? 'none' : 'flex' }} />
                        </TouchableOpacity>
                    </LinearGradient>
                    <View style={{ height: 30 }}></View>
                    <Dialog
                        ...
                        <DialogContent>
                            {
                                <View style={{ width: '100%' }}>
                                    ...
                                    <View style={{ height: 20 }}></View>
                                    <LinearGradient colors={['#00ff00', '#00ff00']} style={styles.linearGradient}>
                ...
                <Header title={true} navigation={this.props.navigation} />
            </Container>
        );
    }
}
