...
export default class propertyCreate extends Component {
  ...
  get_category = (token) => {
    ...
    try {
      fetch(Constant.req_url + "get_rentalcategory", {
        method: 'POST',
        body: JSON.stringify({
          token: token,
        }),
  ...
  adpost = (token) => {
    ...
    console.log('fsdkfslkjdhflkjshdf');
    // var listcategory = this.state.categoryList[this.state.category];
    console.log('here');
    alert(title);
    if (title == null || condition == null || description == null || category == null || location == null || price == 0) { alert('please fill all input!'); console.log('heress'); return }
    ...
    console.log(data)
    try {
      fetch(Constant.req_url + "addproperty", {
        method: 'POST',
        headers: {
      ...
      <Container>
        <Content style={styles.scrollContainer}>
          ...
          <View style={{ marginTop: 10, marginLeft: 60, flexDirection: "row", }}>
            <Image source={{ uri: 'data:image/jpeg;base64,' + this.state.filePath.data }} style={{ width: 50, height: 50, marginLeft: 10 }} />
            <View style={styles.imagebox}></View>
            <View style={styles.imagebox}></View>
            <View style={styles.imagebox}></View>
          </View>
          <InputText txtplaceholder="Property name" onChangeText={(title) => this.setState({ title })} />
          <InputText txtplaceholder="Rental Price" onChangeText={(price) => this.setState({ price })} />
          ...
          <InputText txtplaceholder="Description" onChangeText={(description) => this.setState({ description })} />
          <View style={{ alignItems: 'center' }}>
            <Dropdown
              label='Category'
