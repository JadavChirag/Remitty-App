...
export default class CashoutChooseScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            operators: operators,
    ...
    gettoken11 = () => {
        ...
        if (one) {
            this.props.navigation.navigate('CashoutPaypalScreen');
        }
        else if (two) {
            this.props.navigation.navigate('CashoutBankScreen');
        }
        else {
            this.props.navigation.navigate('CashoutPayoneerScreen');
        }
    }
            ...
            fetch(Constant.req_url + "getconversionpricedet", {
                ...
                .then((res => {
                    ...
                    if (this.state.payment_method == 'Bank Transfer') {
                        this.setState({ feeprice: parseFloat(res.currencyconv_fees_bank).toFixed(0) });
                    } else {
                        this.setState({ feeprice: parseFloat(res.currencyconv_fees_others).toFixed(0) });
                    }
        ...
        return (
            <View style={styles.container}>
                <Content style={{ width: '100%' }}>
                    ...
                    <View style={{
                        textAlign: 'center', alignItems: 'center', borderRadius: 10, backgroundColor: 'white', width: '80%', height: 90, elevation: 5, shadowOffset: { height: 2 },
                    ...
                    {(this.state.payment_method=='Bank Transfer') || (this.state.payment_method=='Paypal') || (this.state.payment_method=='Mobile money') ? (
                        <Text style={[styles.txtsub, { marginTop: 25 }]}>Exchange rate : {'1USD = '+this.state.curprice+''+this.state.tocurrency} </Text>
                    ) : null}
                    <Text style={[styles.txtsub, { marginTop: 25,textAlign:'left',marginLeft:75 }]}>Fees : {this.state.feeprice+' USD'} </Text>
                    <Text style={[styles.txtsub, { marginTop: 30 }]}>
                        Please choose method to cash out.
                     </Text>
                    <TouchableOpacity
                        onPress={() => this.setState({ one: true, two: false, three: false })}
                        style={{
                            flexDirection: 'row',
                            fontSize: 20,
                            alignItems: 'center',
                    ...
                    <TouchableOpacity
                        onPress={() => this.setState({ two: true, one: false, three: false })}
                        style={{
                            flexDirection: 'row',
                            fontSize: 20,
                            alignItems: 'center',
                    ...
                    <TouchableOpacity
                        onPress={() => this.setState({ three: true, one: false, two: false })}
                        style={{
                            flexDirection: 'row',
                            fontSize: 20,
                            alignItems: 'center',
