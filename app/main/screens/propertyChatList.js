...
export default class Stocks extends Component<Props> {
 ...
 gettoken = (name) => {
  ...
  get_chats = (token) => {
    try {
      fetch(Constant.req_url+"getpropertychatlist", {
        method: 'POST',
        body: JSON.stringify({
          token:token,
  ...
  render() {
    return (
      <ScrollView style={styles.scrollContainer}>
        <Text style={styles.textStyle} placeholder='Search stocks'>Chat</Text>
        ...
        <View style={styles.container}>
          {this.state.chatList.map((data, i) => {
            ...
            return <TouchableOpacity onPress={() => this.props.navigation.navigate('propertyChat',{productdetails:JSON.stringify(data)})}><View>
                  <View
                  style={{
                  flexDirection: "row",
                  marginTop: 10,
                  ...
                  >
                      ...
                      <Text style={[styles.buttonText13,{color:'#fe02a2'}]}>{name} </Text>
                      </View>
