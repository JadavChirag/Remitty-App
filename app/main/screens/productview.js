...
import GradiantButton from '../componentItem/productview/GradiantBtn';
...
export default class Stocks extends Component<Props> {
  ...
  get_similarprod = (token) => {
    ...
    try {
      fetch(Constant.req_url + "get_similarprod", {
        method: 'POST',
        body: JSON.stringify({
          token: token,
  ...
  get_suggestprod = (token) => {
    ...
    try {
      fetch(Constant.req_url + "get_suggestprod", {
        method: 'POST',
        body: JSON.stringify({
          token: token,
          user_id: this.state.user_id,
        ...
        <Content>
          <View style={{ paddingTop:0 }}>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <Text style={styles.textStyleright} placeholder='Search stocks'>$ {this.state.productList.price}</Text>
            </View>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <View style={{ marginLeft: 50, width: '100%' }}>
                <GradiantButton
                  textIndo='Chat'
                  onPress={() => this.props.navigation.navigate('marketChat', { productdetails: JSON.stringify(this.state.productList) })}
                />
              </View>
            </View>
            <View
          ...
          <View style={{ textAlign: 'center', alignItems: 'center', marginTop: 0 }}>
            <GradiantButton
              textIndo='Sell faster'
              onPress={() => this.props.navigation.navigate('Package', { productdetails: JSON.stringify(this.state.productList) })}
            />
          </View>
          <Text style={styles.textStyle} placeholder='Search stocks'> Similar Products</Text>
          <View style={[{flexDirection: 'row',
    flexWrap: 'wrap',}]}>
            {
          ...
          <Text style={styles.textStyle} placeholder='Search stocks'> Suggested Products</Text>
          <View style={[ {flexDirection: 'row',
    flexWrap: 'wrap',}]}>
