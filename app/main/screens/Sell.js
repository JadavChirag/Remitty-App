...
import { StyleSheet, Text, View, Image, TextInput, ScrollView, TouchableOpacity, AsyncStorage } from 'react-native';
...
export default class Sell extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      time_in_force: 'day',
      quantity: 0,
      tableHead: ['Symbol', 'Type', 'Quantity', 'Order id', 'Price'],
      tableData: [
      ...
      fetch(urlll, {
        ...
        .then((res => {
          if (res.status) {
            this.setState({
              bar_list: res.res,
              market_price: res.res.response[name][0].l
            });
          }
          else {
            alert(JSON.stringify(res));
            ...
            if (result.length > 0) {
              for (var i = 0; i < result.length; i++) {
                ...
                if (side == 'sell') {
                  ...
                  testdata1.push(result[i].order_id);
  ...
  sellOrder = (name) => {
    var urlll = Constant.req_url + 'api_get_detail/createOrder?user_id=' + this.state.user_id + '&type=Limit&symbol=' + this.state.stock_name + '&stop_price=' + this.state.stop_price + '&limit_price=' + this.state.limit_price + '&time_in_force=' + this.state.time_in_force + '&side=sell&qty=' + this.state.quantity + '&market_price=' + this.state.market_price;
    // alert(urlll);
    try {
      fetch(urlll, {
        method: 'GET',
    ...
    return (
      <ScrollView style={styles.scrollContainer}>
        <View style={styles.container}>
          <Text style={styles.topHeading}>Sell {this.state.stock_name}</Text>
          ...
          <Button
            ...
            title="Sell"
            type="outline"
            onPress={() => this.gettoken('sellOrder')}
          />
